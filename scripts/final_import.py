#!/usr/bin/env python3
"""
Final Data Import Script for Cordly AI - Olist E-commerce Dataset
This script imports CSV files into PostgreSQL database with proper data handling
"""
    ...
    if table_name == 'olist_products_dataset':
        ...
        for col in numeric_columns:
            if col in df.columns:
                ...
                df[col] = df[col].apply(lambda x: int(float(x)) if x is not None and str(x).replace('.', '').replace('-', '').isdigit() else None)
