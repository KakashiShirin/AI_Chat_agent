#!/usr/bin/env python3
"""
Simple Data Import Script for Cordly AI - Olist E-commerce Dataset
This script imports CSV files into PostgreSQL database with basic data cleaning
"""
...
# CSV file mappings - Import in order to respect foreign key constraints
CSV_FILES = [
...
def clean_dataframe(df, table_name):
    ...
    # Special handling for products table - convert float to int
    if table_name == 'olist_products_dataset':
        ...
        for col in numeric_columns:
            if col in df.columns:
                ...
                df[col] = df[col].astype('object')  # Convert to object first
                # Replace NaN with None
                df[col] = df[col].where(pd.notnull(df[col]), None)
                # Convert valid values to int
                df[col] = df[col].apply(lambda x: int(x) if x is not None and str(x).replace('.', '').isdigit() else None)
